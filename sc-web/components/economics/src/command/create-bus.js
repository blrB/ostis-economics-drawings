EconomicsCommandCreateBus = function (source, scene) {
    this.source = source;
    this.scene = scene;
    this.bus = null;
};

EconomicsCommandCreateBus.prototype = {

    constructor: EconomicsCommandCreateBus,
    ...
    execute: function() {
        ...
        if (this.bus == null){
            this.bus = Economics.Creator.createBus(this.source);
            scene.appendBus(this.bus);
            if (scene.drag_line_points.length > 1) this.bus.setPoints(scene.drag_line_points.slice(1));
            var pos = new Economics.Vector2(scene.drag_line_points[0].x, scene.drag_line_points[0].y);
